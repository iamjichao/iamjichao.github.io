...
- **像素**
    ...
    **像素**是指组成图像的小方块，是整个图像中不可分割的最小单位，每个小方块都有一个明确的位置和被分配的色彩值。

- **分辨率**

    **分辨率**是指每一个方向上的像素数量，比如我们常说的 1920×1080 分辨率，就是指横向有 1920 个像素，竖向有 1080 个像素。描述分辨率的单位有： 
...
- **物理分辨率**
    ...
    比如说，一款物理分辨率为 1366x768 的液晶电视，号称**兼容** 1080P 的信号，其实是 1080P 的信号电视可以接收，但输出时只会按照液晶电视 1366x768 的物理分辨率进行输出。只有物理分辨率达到 1920x1080 的液晶电视才能**支持** 1080P 的信号输出。所以买显示器时要分清兼容和支持的区别（跑题了）。
...
以设计稿的尺寸（一般是宽度）为标准，在此基础上，我们定义一个数值代表此时的 1rem 是多少，因为 rem 的本质是等比缩放，我们可以得到：

```
设计稿宽度 / 设计稿参照的 1rem = deviceWidth / 设备对应的 1rem
```

然后我们可以计算出不同设备对应的 1rem 的值：

```
设备对应的 1rem = deviceWidth / (设计稿宽度 / 设计稿参照的 1rem)
```

举个例子🌰：

设计师按照 750px 宽度的标准做设计稿，为了方便计算我们取 100px 为 1rem，那么设计稿的宽度就是 7.5rem，不同设备对应的 1rem 的值就是 deviceWidth / 7.5。iPhone6 横向的CSS 像素为 375px，那么对应的 1rem 数值就是 375 / 7.5=50px。

浏览器渲染页面阶段，在加载 CSS 之前，我们通过 `document.documentElement.clientWidth` 获取到当前设备的 CSS 像素，根据上面的公式动态计算出根元素的 font-size 值，然后把这个值赋给 html 元素，这些通过 JS 动态完成。

```js
document.documentElement.style.fontSize = document.documentElement.clientWidth / 7.5 + 'px';
```

在解析 CSS 时，浏览器会根据 html 元素上 font-size 的值把 rem 值换算成适合自己的 px 值，这样就实现了使用一套 CSS 代码适配不同的设备。
...
那么 CSS 像素作为一个抽象概念，它对于同一设备是不变的吗？
...
也就是说，devicePixelRatio * scale 始终等于 1，那么 device-width 就和设备的物理分辨率始终相等。iPhone 6 的 devicePixelRatio 为 2，当 scale 为 0.5 时，iPhone 6 的横向 CSS 像素就和设计稿一样了。
...
- [针对iPhone的pt、Android的dp、HTML的css像素与dpr、设计尺寸和物理像素的浅分析](http://t.zoukankan.com/wmhuang-p-6817057.html)
