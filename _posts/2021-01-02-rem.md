...
其中，`设计稿宽度 / 设计稿参照的 1rem` 相当于以 1rem 为基准将设计稿平均分成了若干份，那么上边的式子我们可以理解为：把不同设备的视图平均分成和设计稿相同的份数，每一份就是 1rem。

接下来，在浏览器渲染页面阶段，加载 CSS 之前，我们通过 `document.documentElement.clientWidth` 获取到当前设备的 CSS 像素，根据上面的公式动态计算出根元素的 font-size 值，然后把这个值赋给 html 元素，这些通过 JS 动态完成。
...
设计师按照 750px 的标准做设计稿，为了方便计算我们取 100px 为 1rem，那么设计稿的宽度就是 7.5rem，不同设备对应的 1rem 的值就是 deviceWidth / 7.5。

iPhone6 横向的CSS 像素为 375px，那么对应的 1rem 数值就是 375/7.5 = 50px。
