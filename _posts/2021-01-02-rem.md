...
这里的视口是指布局视口（layout viewport），在浏览器中可以通过 `window.innerWidth` 或 `document.documentElement.clientWidth` 获取视口宽度。详细请参考 [视口概念](https://developer.mozilla.org/zh-CN/docs/Web/CSS/Viewport_concepts)。
...
设计师按照 750px 的标准做设计稿，为了方便计算我们取 100px 为 1rem，那么设计稿的宽度就是 7.5rem，不同设备上页面对应的 1rem 的值就是页面视口宽度 / 7.5。
...
淘宝使用了动态设置 scale 的方法，如上述所说根据设备的 devicePixelRatio 动态设置 scale 的值，使页面的视口宽度就和设备的物理分辨率始终相等。还是以 iPhone 6 的 750px 设计稿为例：
